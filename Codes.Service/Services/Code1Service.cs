...
{
    public class Code1Service : ICode1Service
    {
        ...
        public Code1Service(Codes1DbContext context, IBookingService bookingSvc, ILoggerFactory loggerFactory, IMapper mapper)
        {
            _context = context;
        ...
        public async Task<BookingnCommissionViewModel> GetSetBrokerCommissionAdmin(int bookingId)
        {
            BookingnCommissionViewModel model = new BookingnCommissionViewModel();

            try
            {
                BookingCommissionModel bk;

                if (_context.BookingCommissions.Any(a => a.BookingId == bookingId))
                {
                    if (_context.BookingCommissionBreakdowns.Any(a => a.BookingId == bookingId))
                        bk = await _context.BookingCommissions.Where(w => w.BookingId == bookingId).Include(bookingCommission => bookingCommission.BookingCommissionBreakdowns).FirstOrDefaultAsync();
                    else
                        bk = await _context.BookingCommissions.Where(w => w.BookingId == bookingId).FirstOrDefaultAsync();

                    if (bk != null && bk.BookingId > 0)
                    {
                        model = _mapper.Map<BookingCommissionModel, BookingnCommissionViewModel>(bk);
                        /*model = new BookingnCommissionViewModel()
                        {
                            AgentBucketCommission = bk.AgentBucketCommission,
                            AgentBucketPrecentage = bk.AgentBucketPrecentage,
                            BookingId = bk.BookingId,
                            BookingCommissionBreakdowns = bk.BookingCommissionBreakdowns,
                            BookingType = bk.BookingType,
                            ClientCommission = bk.ClientCommission,
                            ClientPrecentage = bk.ClientPrecentage,
                            CompanyBucketCommission = bk.CompanyBucketCommission,
                            CompanyBucketPrecentage = bk.CompanyBucketPrecentage,
                            IsSuccess = true,
                            MemberSavings = bk.MemberSavings,
                            Message = "Success"
                        };*/
                    }
                }
                ...
                {
                    BookingViewModel booking = await _bookingSvc.GetBookingByBookingId(bookingId);

                    if (booking != null && booking.BookingId > 0)
                    {
                        var tmp = await (from a in _context.CodeActivities
                                         join br in _context.Brokers on a.Code.IssuerReference equals br.BrokerId.ToString()
                                         where a.RSIId == booking.RSIId && a.Code.Issuer == "530"
                                         select new
                                         {
                                             a.RSIId,
                                             a.ActivationCode,
                                             CampaignId = a.Code.CampaignId == null ? 0 : (a.Code.CampaignId ?? 0),
                                             ClientId = a.Code.CampaignId == null ? 0 : (a.Code.Campaign.ClientId ?? 0),

                                             BrokerId = br.BrokerId,

                                             PrimaryAgentId = a.Code.CampaignId != null ? a.Code.Campaign.Client.ClientAgents.Count(c => c.Agent.IsActive) > 0 ? a.Code.Campaign.Client.ClientAgents.FirstOrDefault(x => x.Agent.IsActive).AgentId : 0 : 0,

                                             CompanyBucketPrecentage = (br.BrokerCommissionPercentage / 100),
                                             ClientPrecentage = (a.Code.CampaignId == null ? 0 : a.Code.Campaign.Client.CommissionRate / 100),
                                             AgentBucketPrecentage = a.Code.CampaignId == null ? 0 : a.Code.Campaign.Client.ClientAgents.Count(c => c.Agent.IsActive) > 0 ? (a.Code.Campaign.Client.ClientAgents.FirstOrDefault(x => x.Agent.IsActive).CommissionRate / 100) : 0,
                                             AgentCount = a.Code.CampaignId == null ? 0 : a.Code.Campaign.Client.ClientAgents.Count(c => c.Agent.IsActive)

                                         }).FirstOrDefaultAsync();

                        if (tmp != null && tmp.RSIId > 0)
                        {
                            decimal agentBucketCommission = Math.Round(booking.CustomerComparableDiscount * tmp.AgentBucketPrecentage, 2);
                            decimal clientCommission = Math.Round(booking.CustomerComparableDiscount * (decimal)tmp.ClientPrecentage, 2);
                            decimal companyBucketCommission = Math.Round(booking.CustomerComparableDiscount * (decimal)tmp.CompanyBucketPrecentage, 2);

                            model = new BookingnCommissionViewModel()
                            {
                                AgentBucketCommission = agentBucketCommission,
                                AgentBucketPrecentage = (float)tmp.AgentBucketPrecentage,
                                BookingId = booking.BookingId,
                                BookingType = booking.BookingType,
                                ClientCommission = clientCommission,
                                ClientPrecentage = tmp.ClientPrecentage,
                                CompanyBucketCommission = companyBucketCommission,
                                CompanyBucketPrecentage = tmp.CompanyBucketPrecentage,
                                IsSuccess = true,
                                MemberSavings = booking.CustomerComparableDiscount,
                                Message = "Success"
                            };

                            model.BookingCommissionBreakdowns.Add(new BookingCommissionBreakdownViewModel()
                            {
                                Commission = companyBucketCommission - clientCommission - agentBucketCommission,
                                BookingId = booking.BookingId,
                                CommissionPrecentage = tmp.CompanyBucketPrecentage - tmp.ClientPrecentage - (float)tmp.AgentBucketPrecentage,
                                PersonType = "Broker",
                                Id = tmp.BrokerId
                            });
                            ...
                            if (tmp.ClientId > 0)
                            {
                                model.BookingCommissionBreakdowns.Add(new BookingCommissionBreakdownViewModel()
                                {
                                    ...
                                    BookingId = booking.BookingId,
                                    CommissionPrecentage = tmp.ClientPrecentage,
                                    PersonType = "Client",
                                    Id = tmp.ClientId
                                });
                            }
                            ...
                            if (tmp.AgentCount > 0)
                            {
                                if (tmp.AgentCount == 1)
                                {
                                    model.BookingCommissionBreakdowns.Add(new BookingCommissionBreakdownViewModel()
                                    {
                                        Commission = agentBucketCommission,
                                        BookingId = booking.BookingId,
                                        CommissionPrecentage = (float)tmp.AgentBucketPrecentage,
                                        PersonType = "Primary Agent",
                                        Id = tmp.PrimaryAgentId
                                    });
                                }
                                else
                                {
                                    List<ClientAgentModel> agentTmp = await _context.ClientAgents.Where(w => w.ClientId == tmp.ClientId && w.Agent.IsActive).ToListAsync();
                                    float clientAgentCommissionPercentage = (float)agentTmp[tmp.AgentCount - 1].CommissionRate / 100;
                                    decimal clientAgentAmount = Math.Round(booking.CustomerComparableDiscount * (decimal)clientAgentCommissionPercentage, 2);
                                    //BookingCommissionModel bookingModel = new BookingCommissionModel();
                                    if (tmp.AgentCount == 2)
                                    {
                                        model.BookingCommissionBreakdowns.Add(new BookingCommissionBreakdownViewModel()
                                        {
                                            Commission = agentBucketCommission - clientAgentAmount,
                                            BookingId = booking.BookingId,
                                            CommissionPrecentage = (float)tmp.AgentBucketPrecentage - clientAgentCommissionPercentage,
                                            PersonType = "Primary Agent",
                                            Id = tmp.PrimaryAgentId
                                        });
                                        ...
                                        model.BookingCommissionBreakdowns.Add(new BookingCommissionBreakdownViewModel()
                                        {
                                            Commission = clientAgentAmount,
                                            BookingId = booking.BookingId,
                                            CommissionPrecentage = clientAgentCommissionPercentage,
                                            PersonType = "Client Agent",
                                            Id = agentTmp[1].AgentId
                                        });
                                    }
                                    else
                                    {
                                        decimal leftoverAmountAll = agentBucketCommission - clientAgentAmount;

                                        decimal leftoverAmount = Math.Round(leftoverAmountAll / (tmp.AgentCount - 1), 2);

                                        decimal leftoverAmountCheck = Math.Round(leftoverAmount * (tmp.AgentCount - 1), 2);

                                        decimal leftoverPrimaryAgentAmount = leftoverAmount;

                                        if (leftoverAmountAll > leftoverAmountCheck)
                                            leftoverPrimaryAgentAmount += (leftoverAmountAll - leftoverAmountCheck);

                                        float leftoverPrecentageTotal = (float)tmp.AgentBucketPrecentage - clientAgentCommissionPercentage;
                                        float leftoverPrecentagePerPerson = leftoverPrecentageTotal / (tmp.AgentCount - 1);


                                        for (int i = 0; i < (tmp.AgentCount - 1); i++)
                                        {
                                            model.BookingCommissionBreakdowns.Add(new BookingCommissionBreakdownViewModel()
                                            {
                                                Commission = i == 0 ? leftoverPrimaryAgentAmount : leftoverAmount,
                                                BookingId = booking.BookingId,
                                                CommissionPrecentage = leftoverPrecentagePerPerson,
                                                PersonType = i == 0 ? "Primary Agent" : "Agent",
                                                Id = agentTmp[i].AgentId
                                            });
                                        }
                                        ...
                                        model.BookingCommissionBreakdowns.Add(new BookingCommissionBreakdownViewModel()
                                        {
                                            ...
                                            CommissionPrecentage = clientAgentCommissionPercentage,
                                            PersonType = "Client Agent",
                                            Id = agentTmp[tmp.AgentCount - 1].AgentId
                                        });
                                    }
                                }
                            }

                            BookingCommissionModel tmpM = _mapper.Map<BookingnCommissionViewModel, BookingCommissionModel>(model);

                            await _context.BookingCommissions.AddAsync(tmpM);
                            await _context.SaveChangesAsync();

                            model = _mapper.Map<BookingCommissionModel, BookingnCommissionViewModel>(tmpM);
                        }
                    ...
                    {
                        model.Message = "BookingId not found";
        ...
        public async Task<Codes1.Service.ViewModels.ListViewModel<CampaignViewModel>> GetCampaigns(int brokerId, int startRowIndex = 0, int numberOfRows = 10, string sortColumn = "DEFAULT")
        {
            Codes1.Service.ViewModels.ListViewModel<CampaignViewModel> items = new Codes1.Service.ViewModels.ListViewModel<CampaignViewModel>();
...
#pragma warning restore 1998
        ...
        public async Task<DataTableViewModel<ClientListViewModel>> GetClients(int brokerId)
        {
            ...
            try
            {
                ...
                var tmp = from a in _context.Clients where a.BrokerId == brokerId && a.IsActive == true select a;
